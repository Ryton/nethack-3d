...
const char *name, *mode;
{
    ...
    /* Try nhdat archive first (if initialized), then fall back to direct file.
     * WASM-DEV: removed early !dlb_initialized bail-out so standalone embedded
     * files (quest.dat, data, etc.) can be found even if nhdat open failed. */
