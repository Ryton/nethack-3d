...
makerooms()
{
    ...
    int wasm_iter = 0;
    int wasm_prev_nroom = -1;
    int wasm_stuck = 0;
    fprintf(stderr, "[WASM] makerooms: ENTER nroom=%d\n", nroom);
#endif
...
#ifdef __EMSCRIPTEN__
        wasm_iter++;
        if (nroom == wasm_prev_nroom) {
            wasm_stuck++;
        } else {
            wasm_stuck = 0;
            wasm_prev_nroom = nroom;
        }
        if (wasm_stuck > 20) {
            fprintf(stderr, "[WASM] makerooms: bailing after %d stuck iterations at nroom=%d\n", wasm_stuck, nroom);
            return;
        }
        if (wasm_iter > 500) {
            fprintf(stderr, "[WASM] makerooms: bailing after %d total iterations at nroom=%d\n", wasm_iter, nroom);
            return;
        }
#endif
...
#ifdef __EMSCRIPTEN__
            /* WASM: vault .lev loading isn't producing rooms (silent
               failure in sp_level_loader/coder due to long-size mismatch
               somewhere). Bypass vault path and use regular rooms. */
            fnam = NULL;
#endif

            if (fnam) {
#ifdef __EMSCRIPTEN__
                fprintf(stderr, "[WASM] makerooms: attempting vault '%s' (iter=%d nroom=%d)\n", fnam, wasm_iter, nroom);
#endif
