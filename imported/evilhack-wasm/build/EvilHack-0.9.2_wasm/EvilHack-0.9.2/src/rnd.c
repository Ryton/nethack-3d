...
int x;
{
    ...
    if (x <= 0) {
        ...
        static int wasm_rn2_zero_count = 0;
        if (++wasm_rn2_zero_count <= 5) {
            void *ra = __builtin_return_address(0);
            fprintf(stderr, "[WASM] rn2(%d) #%d caller_ra=%p\n", x, wasm_rn2_zero_count, ra);
        } else if (wasm_rn2_zero_count == 6) {
            fprintf(stderr, "[WASM] rn2(0): suppressing further warnings\n");
        }
        return 0; /* silent return — no pline spam */
#else
